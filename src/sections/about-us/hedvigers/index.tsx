...
export const Hedvigers: React.SFC<HedvigersProps> = ({ teamtailorUsers }) =>
    <Container actions={actions} initialState={{ selectedImage: teamtailorUsers[0].picture.large, users: teamtailorUsers.filter(user => user.picture) } as State}>
        {({ selectedImage, users, setSelectedImage }) => (
            <Box>
                <List users={users} onSelect={user => setSelectedImage(user.picture.large)} selectedImage={selectedImage} />
                <SelectedImage images={users.map(user => user.picture.large)} selectedImage={selectedImage} />
            </Box>
        )}
    </Container>
